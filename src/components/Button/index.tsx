...
export function Button({
  ...
  const buttonVariants: Record<ButtonVariants, string> = {
    ...
    ghost: clsx('bg-slate-300 text-slate-400 hover:bg-slate-700'),
  ...
  const buttonClasses = `${buttonVariants[variant]} ${buttonSizes[size]} flex items-center justify-center cursor-pointer transition disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed ${props.className}`;
  return (
